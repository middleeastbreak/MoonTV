...
const DownloadManager = ({ isOpen, onClose }: DownloadManagerProps) => {
  ...
  // 查看片段的任务ID
  const [viewingSegmentsTaskId, setViewingSegmentsTaskId] = useState<
    string | null
  >(null);
  ...
  const batchDownloadActiveRef = useRef(false);
    ...
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        try {
          ...
          const processedTasks = savedTasks.map(
            (t: DownloadTask & { _originalStatus?: string }) => {
              // 使用 _originalStatus 判断是否需要自动恢复
              const wasDownloading =
                t._originalStatus === 'downloading' ||
    ...
    if (typeof window !== 'undefined') {
      const tasksToSave = tasks.map(
        ({
          ...
          pauseResumeController: _pauseResumeController,
          config,
          ...rest
        }) => ({
          ...rest,
  ...
  const executeDownload = useCallback(
    async (
      ...
      try {
        ...
        } else if (hasFailedSegments) {
          // 防御性处理：流式下载不允许把含缺失片段的文件标记为完成。
          setTasks((prev) =>
            prev.map((t) =>
              t.id === taskId
                ? {
        ...
        if (error instanceof Error && error.message === '下载已取消') {
          ...
          if (
            taskAfterError?.isEarlyCompleting ||
            taskAfterError?.status === 'completed'
          ) {
  ...
  // 从配置创建并开始下载任务
  const addTaskFromConfig = useCallback(
    (config: {
      url: string;
      title: string;
      downloadType: 'TS' | 'MP4';
  ...
  // 整季任务按集串行处理，避免多集同时下载占满带宽和内存。
  useEffect(() => {
    let cancelled = false;
    const handleAddBatchEvent = async (event: Event) => {
      if (batchDownloadActiveRef.current) {
        // 防止重复事件或连续点击创建两份相同的整季任务。
        void Swal.fire({
          icon: 'info',
          title: '整季下载正在进行',
          text: '请等待当前整季任务结束后再添加新的整季任务。',
          customClass: { container: 'z-[11000]' },
        });
        return;
      }
      batchDownloadActiveRef.current = true;

      const config = (event as CustomEvent).detail as {
      ...
      try {
        await runSeasonDownloadQueue(queue, async (episode, index) => {
          ...
          try {
            ...
            if (cancelled) return;
            parsedTask.title = episode.title;
            parsedTask.type = config.downloadType;
            const controller = new AbortController();
            const pauseResumeController = new PauseResumeController();
            const completeStreamRef = {
              current: null as (() => Promise<void>) | null,
        ...
        batchDownloadActiveRef.current = false;
      }
    };
    ...
    return () => {
      cancelled = true;
      window.removeEventListener('addDownloadBatch', handleAddBatchEvent);
    };
  }, [executeDownload]);

  // 执行下载任务（从任务配置启动）
  const startTaskDownload = useCallback(
    async (taskId: string, parsedTask: M3U8Task) => {
  ...
  const resumeTask = useCallback(
    async (taskId: string) => {
      ...
      if (
        taskToResume.status === 'downloading' &&
        taskToResume.abortController &&
        hasFailedSegments
      ) {
        // eslint-disable-next-line no-console
      ...
      if (
        taskToResume.pauseResumeController &&
        taskToResume.pauseResumeController.getPaused()
      ) {
        // eslint-disable-next-line no-console
      ...
      const parsedTaskToUse =
        taskToResume.parsedTask || taskToResume.config?.parsedTask;
      ...
      if (parsedTaskToUse) {
        ...
        if (pendingCount === 0) {
          ...
          if (errorCount === 0) {
            ...
            setTimeout(async () => {
              try {
                ...
                if (downloadType === 'MP4') {
                  ...
                  const rangeDuration =
                    (totalInRange / totalSegmentsCount) * totalDuration;
            ...
            if (isStreamMode) {
              // 已中止的流无法补写旧文件，用户继续时从头创建完整文件。
              resetStreamingDownloadProgress(parsedTaskToUse);
              setTasks((prev) =>
                prev.map((t) =>
                  t.id === taskId
                    ? {
            ...
            t.id === taskId
              ? {
                  ...
                  config: {
                    ...
                    endSegment:
                      t.config?.endSegment || parsedTask.tsUrlList.length,
                        ...
                        <div className='flex flex-wrap gap-2 mt-2'>
                          ...
                          <span className='inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400'>
                            {getEffectiveDownloadConcurrency(
                              ...
                              task.config.streamMode || 'disabled'
                            )}{' '}
                            线程
                          </span>
                          {task.config.rangeMode &&
                            (() => {
                              ...
                              if (
                                task.parsedTask &&
                                Array.isArray(task.parsedTask.segmentDurations)
                              ) {
                              ...
                              return (
                                <span className='inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400'>
                                  ...
                                  {task.parsedTask &&
                                    task.parsedTask.segmentDurations &&
                                    task.parsedTask.segmentDurations.length >
                                      0 && (
                                      <>
                                        &nbsp;|&nbsp;时长:{' '}
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                const { startSegment, endSegment } =
                                  task.parsedTask.rangeDownload;
                                const downloadType =
                                  task.config?.downloadType || 'TS';
                                // 按顺序收集已下载片段
                                const segments: ArrayBuffer[] = [];
                                for (
                                  let i = startSegment - 1;
                                  i < endSegment;
                                  i++
                                ) {
                                  const segment =
                                    task.parsedTask.downloadedSegments?.get(i);
                                ...
                                if (downloadType === 'MP4') {
                                  const totalDuration =
                                    task.parsedTask.durationSecond || 0;
                                  const totalSegments =
                                    endSegment - startSegment + 1;
                                  const rangeDuration =
                                    (totalDuration / totalSegments) *
                                    segments.length;
                                  blob = transmuxTSToMP4(
                                    segments,
                                    rangeDuration
                                  );
                                } else {
        ...
        (() => {
          ...
          return task?.parsedTask ? (
            <SegmentViewer
              ...
              taskExists={() =>
                tasks.some((t) => t.id === viewingSegmentsTaskId)
              }
                    ...
                    if (t.id === viewingSegmentsTaskId && t.parsedTask) {
                      ...
                      for (let i = startSegment - 1; i < endSegment; i++) {
                        if (t.parsedTask.finishList[i].status === 'success') {
                          ...
                          t.parsedTask.finishList[i].status === 'error'
                        ) {
                          errorCount++;
                        }
                      }
