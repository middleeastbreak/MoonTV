...
1. 用户菜单的版本信息显示 `4.1.0`。
2. 搜索“芙利莲”，原搜索无直接结果时出现“可能想找”；关闭“智能搜索纠错”后不再出现。
3. “葬送的芙莉莲 第二季”和“葬送的芙莉莲第二季”合并；“问心”和“问心2”分开；电影解说不并入正片。
4. 手机或 iPad 首次打开本版时自动弹幕默认关闭；手动开启显示耗电提示，弹幕透明度默认 75%。
5. 播放线路彻底恢复失败后出现 5 秒换源倒计时；可取消，最多尝试两个备用源。
6. 播放器设置中的“播放诊断”可复制脱敏信息，内容不应包含播放 URL、Token、账号或密码。
7. 下载弹窗显示“一键下载整季（N 集）”；确认后各集按顺序进入任务，每集独立保存，失败集单独标记。

整季下载依赖浏览器保持页面打开。浏览器可能对连续保存多个文件弹出授权提示，请允许该站点下载多个文件。大文件建议选择 Service Worker 流式下载或文件系统直写。
