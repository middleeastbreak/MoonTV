# MoonTV 4.1.3 Docker 直接部署说明

本说明适用于不使用 Docker Compose、直接运行 Docker 容器，并由宿主机 Nginx 提供 HTTPS 反向代理的环境。交付镜像为 Linux AMD64 Docker 归档，导入后的本地标签为 `moontv:4.1.3`，应用内版本显示为 `4.1.3`。
...
从 [v4.1.3 Release](https://github.com/middleeastbreak/MoonTV/releases/tag/v4.1.3) 下载以下两个文件，并放在同一目录：
...
docker image inspect moontv:4.1.3 \
...
- 弱网环境自动换源默认关闭，可在右上角用户菜单的“本地设置”中开启。
- 自动换源会核对分集信息，无法唯一确认时停止切换；每次播放最多尝试两个备用源。
- 首次播放和切换集数会主动尝试播放；浏览器禁止带声音自动播放时会提示点击播放。
