# MoonTV 4.1.0 Docker 直接部署说明

本说明适用于不使用 Docker Compose、直接运行 Docker 容器，并由宿主机 Nginx 提供 HTTPS 反向代理的环境。交付镜像为 Linux AMD64 Docker 归档，导入后的本地标签为 `moontv:4.1.0`，应用内版本显示为 `4.1.0`。
...
从 [v4.1.0 Release](https://github.com/middleeastbreak/MoonTV/releases/tag/v4.1.0) 下载以下两个文件，并放在同一目录：

- `moontv-4.1.0-linux-amd64.tar`
- `moontv-4.1.0-linux-amd64.tar.sha256`

```bash
sha256sum -c moontv-4.1.0-linux-amd64.tar.sha256
docker load -i moontv-4.1.0-linux-amd64.tar
docker image inspect moontv:4.1.0 \
...
不使用弹幕时可以删除 `location /danmu-api/`。宿主机系统代理不影响 Nginx 到本机 `127.0.0.1:9321` 的通信；命令行测试受代理变量影响时可使用 `curl --noproxy '*'`。
...
整季下载在支持目录访问的桌面 Chrome/Edge 中只询问一次目录，并按集串行保存；手机和 iPad 不提供整季批量保存，以避免 Safari 内存和文件保存限制。单集下载仍可使用浏览器支持的模式。

更新镜像时，先拉取新标签，再使用原有环境变量和挂载参数重新创建容器。不要仅执行 `docker restart`，因为重启不会更换镜像。
